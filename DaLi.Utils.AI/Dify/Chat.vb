...
Namespace Dify
	...
	Public Class Chat
		...
		Public Sub New(Optional url As String = "", Optional key As String = "", Optional user As String = "")
			MyBase.New(url, key.EmptyValue(AISettings.DIFY_KEY_CHAT))

			_User = user.EmptyValue(Assembly.GetEntryAssembly.Title)
		End Sub

		''' <summary>开始新的消息队列</summary>
		Public Sub NewChat(Optional user As String = "")
			_TaskId = ""
			_User = user.EmptyValue(Assembly.GetEntryAssembly.Title)
		End Sub
		...
		Public Function Process(prompt As String, Optional params As KeyValueDictionary = Nothing, Optional images As String() = Nothing) As ChatResult
			If prompt.IsEmpty Then Return Nothing

			Dim options As New KeyValueDictionary From {{"query", prompt}, {"inputs", params}, {"user", _User}, {"conversation_id", _TaskId}, {"response_mode", "blocking"}}
			...
			Dim res = Execute(Of Model.Chat.Response)("/v1/chat-messages", HttpMethodEnum.POST, options, status)
