...
Namespace Auto.Rule
	...
	''' <summary>Dify 知识库文档段落列表</summary>
...
#Region "EXECUTE"
		...
		Protected Overrides Function ExecuteRule(data As IDictionary(Of String, Object), message As AutoMessage) As Object
			...
			Dim res = ai.Segment_Query(id, docId, Keyword)

			If res IsNot Nothing AndAlso res.Data IsNot Nothing AndAlso res.Data.Data.NotEmpty Then
				message.SetSuccess()
				Return res.Data.Data.Select(Function(x) New With {x.ID, x.Content, x.Answer, x.Keywords, x.Enabled}).ToList
			Else
